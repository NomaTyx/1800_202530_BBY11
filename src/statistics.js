...
document
  .getElementById("getDataForWhite")
  .addEventListener("click", () => displayTournamentData("white"));
document
  .getElementById("getDataForBlack")
  .addEventListener("click", () => displayTournamentData("black"));

let resultData;

async function readResultForWhite() {
  const userData = user.uid;
  const userTournamentDataRef = collection(db, "users", userData, "tournamentData").where(
    "color",
    "==",
    "white"
  );
